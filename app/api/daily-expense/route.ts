...
export async function POST(request: Request) {
  ...
  try {
    ...
    const operatorExpense =
      body.operatorGeneral * RATES.operatorGeneral +
      body.operatorOvertime * RATES.operatorOvertime;

    const helperExpense =
      body.helperGeneral * RATES.helperGeneral +
      body.helperOvertime * RATES.helperOvertime;

    const cuttingExpense =
      body.cuttingGeneral * RATES.cuttingGeneral +
      body.cuttingOvertime * RATES.cuttingOvertime;

    const finishingExpense =
      body.finishingGeneral * RATES.finishingGeneral +
      body.finishingOvertime * RATES.finishingOvertime;

    const qualityExpense =
      body.qualityGeneral * RATES.qualityGeneral +
    ...
    return NextResponse.json(
      { error: error.message || "Failed to create daily expense" },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error.message || "Failed to update daily expense" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error.message || "Failed to delete daily expense" },
      { status: 500 }
    );
  }
}
